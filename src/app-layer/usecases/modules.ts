...
import { ALL_MODULES, resolveEnabledModules } from '@/lib/modules';
...
export async function setEnabledModules(ctx: RequestContext, enabledModules: ModuleKey[]) {
    assertCanAdmin(ctx);
    return runInTenantContext(ctx, async (db) => {
        const row = await ModuleSettingsRepository.upsert(db, ctx, enabledModules);
        await logEvent(db, ctx, {
            action: 'TENANT_MODULES_UPDATED',
            entityType: 'TenantModuleSettings',
            entityId: row.id,
            details: `Enabled modules set to: ${enabledModules.join(', ') || '(none)'}`,
            detailsJson: {
